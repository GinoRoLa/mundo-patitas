  ...
  public function cerrarRecaudacion(array $cab, array $rowsDetalle): array
{
  ...
  $mER                    = (float)$cab['MontoEsperadoRetorno'];
  ...
  try {
    ...
    $sqlCntFaltas = "
      ...
      WHERE Id_Trabajador = {$idTrabajadorRepartidor}
        AND Estado = 'Faltante'
    ";
    $rsCnt = mysqli_query($this->cn, $sqlCntFaltas);
    if (!$rsCnt) {
      throw new Exception('Error contando faltas de recaudación: ' . mysqli_error($this->cn));
    }
    ...
    if ($faltas > $maxFaltas) {
      $sqlSusp = "
        UPDATE t16CatalogoTrabajadores
        SET Estado = 'Suspendido'
        WHERE id_Trabajador = {$idTrabajadorRepartidor}
      ";
      if (!mysqli_query($this->cn, $sqlSusp)) {
        throw new Exception('Error suspendiendo repartidor en t16CatalogoTrabajadores: ' . mysqli_error($this->cn));
      }
      $repartidorSuspendido = true;
    }
