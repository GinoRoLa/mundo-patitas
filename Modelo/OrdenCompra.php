...
public function crearOCsDesdeAdjudicacion($idEval, array $adjud): array
{
  ...
  // ✅ Validar que la evaluación exista y esté Aprobada/Parcialmente Aprobada
  if (!$this->evaluacionEsValida($idEval)) {
    throw new Exception("Evaluación no está en estado Aprobado/Parcialmente Aprobado.");
  }

  // 1) Normalizar adjudicación a formato por proveedor
  ...
  if (!empty($erroresCobertura)) {
    // Puedes formatear mejor el mensaje si quieres
  ...
  try {
    ...
    foreach ($porProv as $prov) {
      ...
      // 2) Calcular totales con %IGV (18% por defecto).
      $porcIGV = 18.00;
      $sub = 0.0;
      foreach ($items as $it) {
      ...
      // 3) Fingerprint estable usando Id_ReqEvaluacion
      $fpData = [
        'idEval' => $idEval,
        'ruc'    => $ruc,
      ...
      // 4) Idempotencia: por Huella si existe; si no, heurística por Id_ReqEvaluacion+RUC+Total
      if ($t06TieneHuella) {
      ...
      // 5) Insert cabecera t06 (con Id_ReqEvaluacion)
      if ($t06TieneHuella) {
        $sqlCab = "INSERT INTO t06OrdenCompra
          (Fec_Emision, RUC_Proveedor, RazonSocial, Id_ReqEvaluacion, Moneda,
           PorcentajeIGV, SubTotal, Impuesto, MontoTotal, Estado, Huella)
          VALUES (NOW(), ?, ?, ?, 'PEN', ?, 0, 0, 0, 'Emitida', ?)";
        $st = mysqli_prepare($this->cn, $sqlCab);
        mysqli_stmt_bind_param($st, "ssids", $ruc, $razon, $idEval, $porcIGV, $finger);
      } else {
        $sqlCab = "INSERT INTO t06OrdenCompra
          (Fec_Emision, RUC_Proveedor, RazonSocial, Id_ReqEvaluacion, Moneda,
          ...
          VALUES (NOW(), ?, ?, ?, 'PEN', ?, 0, 0, 0, 'Emitida')";
        $st = mysqli_prepare($this->cn, $sqlCab);
        mysqli_stmt_bind_param($st, "ssid", $ruc, $razon, $idEval, $porcIGV);
      }
      mysqli_stmt_execute($st);
      $idOC = (int)mysqli_insert_id($this->cn);
      mysqli_stmt_close($st);

      // 6) Insert detalle t07
  ...
  {
    // Cabecera + proveedor + requerimiento via t407
    $qCab = "SELECT 
                oc.Id_OrdenCompra,
                oc.Fec_Emision,
                oc.RUC_Proveedor,
    ...
    // Datos de la empresa (hardcodeado por ahora)
    $empresa = [
