...
const cargarDetalleOrdenPedido = async (ordenId, contenedor) => {
  if (contenedor.dataset.loaded === "1") return;
...
const recalcularResumen = () => {
  if ($montoBruto) $montoBruto.textContent = toMoney(resumenState.bruto);
  if ($montoDescuento) $montoDescuento.textContent = toMoney(resumenState.descuento);
  if ($montoTotal) $montoTotal.textContent = toMoney(resumenState.bruto - resumenState.descuento);
};
...
const getTotal = () => {
  if (typeof getTotalAPagar === "function") return Number(getTotalAPagar()) || 0;
  if (typeof obtenerTotal === "function") return Number(obtenerTotal()) || 0;
